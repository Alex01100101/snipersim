...
static PyObject *
updateHistory(PyObject *self, PyObject *args)
{
   long int core_id = -999;
   bool state = false;
   if (!PyArg_ParseTuple(args, "ll", &core_id, &state))
       return NULL;
   Sim()->getMagicServer()->updateHistory(core_id, state);
   Py_RETURN_NONE;
}
...
static PyMethodDef PyDvfsMethods[] = {
   ...
   {"update_history", updateHistory, METH_VARARGS, "Update History"},
